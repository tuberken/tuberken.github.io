...
// statements in JavaScript script executed sequentially from top to bottom. If the processing 
// logic require so, the sequential flow of execution can be altered in two ways:
...
// as long as a certain expression is true. In this section, we will cover if, else , else if statements. The comparison and logical operators we learned in the previous sections will be useful in here.
...
// falsy condition, we should have another block, which is going to be else.
...
// have more than two conditions? In that case, we would use else if conditions.
// If Else if Else
...
// conditions. As similar to our daily life, programming is also full of conditions.
...
// Switch is an alternative for if else if else else. The switch statement starts
// with a switch keyword followed by a parenthesis and code block. Inside the code
// block we will have different cases. Case block runs if the value in the switch
// statement parenthesis matches with the case value. The break statement is to 
// terminate execution so the code execution does not go down after the condition
// is satisfied. The default block runs if all the cases don't satisfy the condition.
